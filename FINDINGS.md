...
- [ ] **USPTO patent assignment records for the Mitradyne family.** The single most likely place a university stake or security interest would appear. **BLOCKER IDENTIFIED: needs a free Open Data Portal API key from data.uspto.gov.** The legacy assignment API was decommissioned 2026-06-05; `api.uspto.gov` returns HTTP 401 Unauthorized without a key. One signup form, same as the FEC key
