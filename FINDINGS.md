...
- [ ] **Ontario Business Registry search for Mitradyne** (free, needs a browser). Returns **directors**, registered office and incorporation date. If a Guelph commercialisation officer sits on the board that is a direct answer. **Best value of the three routes**
- [ ] **EPO OPS credentials** (free, email only, `developers.epo.org/user/register`, non-paying tier) to query the European Patent Register for EP4538367 applicant history and transfers. No identity verification. Automatable here once issued
- [ ] ~~USPTO ODP key for assignment records~~ **RECOMMEND AGAINST.** Requires ID.me with government photo ID and SSN, and assignment recordation captures conveyances only. **Equity and licences are not recorded**, so it probably would not answer the question. Finding 25 addendum 3
